...
- **合同模块**
  ...
  - 获取：`GET /api/contracts/{id}`（注意：当前实现未做归属校验，存在越权风险；删除接口已做校验）
...
- `GET /api/contracts/{id}` 获取合同详情（当前未做归属校验）
...
## 下一步计划（建议）
1) 增加合同详情接口的归属校验（修复潜在越权读取）
2) 知识库数据治理：导入工具/模板、doc_type 规范化、按类型统计
3) 分析历史聚合：按用户列出分析结果摘要、支持再次打开最近结果
4) GraphRAG（Neo4j）接入：图谱检索结果与向量检索融合（RRF 重排序）
